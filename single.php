  ...
  <?php if(have_posts()):?>
    <?php while(have_posts()): the_post();?>
      ...
      <div class="single-blog-post <?php if($post_type == 'projects') { echo 'cpt-project'; }?>">
        <?php if ($prev_post) : ?>
          <a class="cpt-project__arrow cpt-project__arrow--prev" href="<?php echo get_permalink($prev_post->ID); ?>">
          </a>
        <?php endif; ?>
        <?php if ($next_post) : ?>
          <a class="cpt-project__arrow cpt-project__arrow--next" href="<?php echo get_permalink($next_post->ID); ?>">
          </a>
        <?php endif; ?>
              ...
              <div class="row cpt-project__row">
                <div class="col-xl-6 cpt-project__column">
                  <?php if(!empty(get_post_thumbnail_id($post->ID))):?>
                    <div class="cpt-project__image cpt-project__image--main">
                      <?php echo wp_get_attachment_image(get_post_thumbnail_id($post->ID), 'large', '', ["class" => "object-fit-contain"]); ?>
                    </div>
                  <?php endif;?>
                </div>
                <div class="col-xl-3">
                  ...
                  <div class="cpt-project__sidebar">
                    <div>
                      ...
                      <?php if(!empty($services_proviced)):?>
                        <div>
                          <?php echo apply_filters('the_title', $services_proviced);?>
                        </div>
                      <?php endif;?>
                    </div>
                    <?php if(!empty($project_gallery)):?>
                      <div class="cpt-project__gallery">
                        <?php foreach($project_gallery as $key => $item):?>
                          <div class="cpt-project__image cpt-project__image--small">
                ...
                <div class="col-12">
                  <div class="single-blog-post__content">
                    <div class="single-blog-post__image">
                      <?php echo wp_get_attachment_image(get_post_thumbnail_id($post->ID), 'large', '', ["class" => "object-fit-cover"]); ?>
                    </div>
                    <h1 class="single-blog-post__title"><?php the_title();?></h1>
                    <div class="single-blog-post__text">
                      <?php the_content(); ?>
                    </div>
